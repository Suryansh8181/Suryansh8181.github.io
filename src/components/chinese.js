...
import Restaurants from "./chineseRestaurants.json";
...
function Chinese(){
    ...
    axios.get("https://foodobackend.herokuapp.com/searchResultsbyCuisine/Chinese")
    .then(response=>{
        localStorage.setItem("chinese",JSON.stringify(response.data))
    })
    const usera=localStorage.getItem("chinese");
    const userb=JSON.parse(usera)
    return(
        <>
        <NavigationBar />
        <div style={{ paddingTop: "8%" }}>
        <div style={{ marginLeft: '2rem' }} ><h3>Popular Chinese Restraunts in {ucity}</h3></div>
       <div className='chinese'  style={{display: 'flex',margin:'30px'}}>
